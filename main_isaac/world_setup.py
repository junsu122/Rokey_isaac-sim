...
각 섹션 슬롯 07~09 (3개) 에 드론 픽업용 ArUco 박스가 사전 배치된다.
"""
import omni.usd
...
# 드론 픽업용 박스 슬롯: 9-slot section 안쪽의 후반 슬롯 07~09
_SECTION_BOX_SLOTS = list(range(7, 10))
...
def setup_warehouse(world) -> None:
    ...
    # 물리 지면 평면 추가 — 드론/로봇이 바닥 아래로 추락하지 않도록 z=0에 무한 충돌 평면 생성
    try:
        world.scene.add_default_ground_plane()
        print("[WorldSetup] 물리 지면 평면(Ground Plane) 추가 완료 (z=0)")
    except Exception as _gp_err:
        print(f"[WorldSetup] 지면 평면 추가 실패 (창고 USD에 이미 포함됐을 수 있음): {_gp_err}")
    ...
    # Section A / B / C  9-slot 격자 — 슬롯 #01 은 IW Hub 배달용으로 비워둠
    ...
    # 섹션 박스 사전 배치 (드론 픽업용: 슬롯 07/08/09)
    _spawn_section_boxes(stage)


def _spawn_section_boxes(stage) -> None:
    """섹션 A/B/C 의 슬롯 07~09 각 pod 에 30cm ArUco 박스 3개 적층 배치.
    ...
    for sec, positions in SECTION_PODS.items():
        ...
        for slot in _SECTION_BOX_SLOTS:
            pos_idx = slot - 1      # 0-based: slot 07 → index 6
    ...
    print(f"[WorldSetup] 섹션 박스 {spawned}개 사전 배치 완료 "
          f"(슬롯 07~09, 3개 적층×섹션 A/B/C, 30cm ArUco)")
...
class BoxSpawner:
    ...
    def _spawn_next(self) -> None:
        ...
        try:
            ...
            UsdPhysics.MassAPI.Apply(prim).GetMassAttr().Set(2.0)
