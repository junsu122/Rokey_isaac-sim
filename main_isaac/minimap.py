...
_C_ARUCO_ZONE = (0, 255, 255)   # Spot ArUco placement zones near Section A
...
class Minimap:
    ...
    def __init__(self, agents: list):
        ...
        self._pod_counter = 0
        self._click_box_id_cache: dict[str, str] = {}
    ...
    def _build_static_bg(self) -> np.ndarray:
        ...
        # Spot ArUco placement zones next to Section A.
        # Draw from robot_config so Spot placement and minimap markings stay aligned.
        aruco_goals = None
        for cfg in C.ROBOT_REGISTRY:
            if cfg.get("type") == "spot" and cfg.get("aruco_goals"):
                aruco_goals = cfg["aruco_goals"]
                break
        if aruco_goals:
            for aid, goal_xy in sorted(aruco_goals.items()):
                gx, gy = float(goal_xy[0]), float(goal_xy[1])
                px, py = _w2p(gx, gy)
                half = 9
                cv2.rectangle(
                    img, (px - half, py - half), (px + half, py + half),
                    _C_ARUCO_ZONE, 2, cv2.LINE_AA,
                )
                cv2.circle(img, (px, py), 4, _C_ARUCO_ZONE, -1, cv2.LINE_AA)
                cv2.putText(img, f"A-ID{aid}", (px - 17, py - 13),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.28, _C_ARUCO_ZONE, 1, cv2.LINE_AA)
            label_x, label_y = _w2p(0.0, 15.35)
            cv2.putText(img, "SPOT ARUCO ZONE", (label_x - 55, label_y),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.35, _C_ARUCO_ZONE, 1, cv2.LINE_AA)
    ...
    def _draw_dynamic(self, base: np.ndarray) -> np.ndarray:
        ...
        world_children = list(world_root.GetChildren()) if world_root.IsValid() else []
        ...
        for prim in _children("/World/DynamicBoxes"):
            try:
        ...
        seen_pods = set()
        seen_click_boxes = set()
        for prim in world_children:
            path = str(prim.GetPath())
            if path.startswith("/World/AutoBox_"):
                tr = cache.GetLocalToWorldTransform(prim).ExtractTranslation()
                px, py = _w2p(float(tr[0]), float(tr[1]))
            ...
            elif ("Shelf" in path or "shelf" in path):
                tr = cache.GetLocalToWorldTransform(prim).ExtractTranslation()
                px, py = _w2p(float(tr[0]), float(tr[1]))
                cv2.rectangle(img, (px - 9, py - 6), (px + 9, py + 6), _C_PODSTACK, 1)

        for prim in _children("/World/MinimapClickBoxes"):
            path = str(prim.GetPath())
            try:
                seen_click_boxes.add(path)
                tr = cache.GetLocalToWorldTransform(prim).ExtractTranslation()
                px, py = _w2p(float(tr[0]), float(tr[1]))
                box_id = self._click_box_id_cache.get(path)
                if box_id is None:
                    box_id = "C"
                    tex_prim = stage.GetPrimAtPath(f"{path}/aruco_mat/Texture")
                    tex_attr = tex_prim.GetAttribute("inputs:file") if tex_prim.IsValid() else None
                    tex_val = str(tex_attr.Get()) if tex_attr else ""
                    if "aruco_id" in tex_val:
                        box_id = tex_val.split("aruco_id", 1)[1].split(".png", 1)[0]
                    self._click_box_id_cache[path] = box_id
                cv2.rectangle(img, (px - 6, py - 6), (px + 6, py + 6), _C_POD_CLICK, -1)
                cv2.rectangle(img, (px - 6, py - 6), (px + 6, py + 6), (255, 230, 120), 1)
                cv2.putText(img, box_id, (px - 4, py + 4),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.28, (0, 0, 0), 1, cv2.LINE_AA)
            except Exception:
                pass

        for prim in _children("/World/PodStacks"):
            path = str(prim.GetPath())
            try:
                seen_pods.add(path)
                tr = cache.GetLocalToWorldTransform(prim).ExtractTranslation()
        ...
        for wx, wy, prim_path in self._click_pods:
            if prim_path in seen_click_boxes:
                continue
            px, py = _w2p(wx, wy)
        ...
        for agent in self._agents:
            ...
            try:
                ...
                _ms_short = {"IDLE":"IDLE","TAKEOFF":"UP","FLY_PICK":"PICK",
                             "DESCEND_PICK":"DPICK","GRAB":"GRAB","ASCEND_PICK":"UP",
        ...
        # 타이틀 / 범례
        cv2.putText(img, "Warehouse Minimap  [Right click: spawn box]", (8, 18),
    ...
    def update(self) -> None:
        """렌더 루프에서 매 step 호출. 10 프레임마다 전송 (~5 Hz)."""
        self._frame_skip += 1
