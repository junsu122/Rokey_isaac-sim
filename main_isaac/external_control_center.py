...
class ExternalControlCenter:
    ...
    def _build_box_tab(self):
        ...
        ttk.Spinbox(row, textvariable=self.box_aruco, from_=0, to=2, width=9).pack(side=tk.LEFT)
    ...
    def _ensure_drone(self, name: str):
        ...
        # ── 자율 미션 입력 ──────────────────────────────────────────
        mission_frame = ttk.LabelFrame(frame, text="Auto Mission", padding=6)
        mission_frame.pack(fill=tk.X, pady=(10, 0))
        ttk.Label(
            mission_frame,
            text="Targets  (e.g. A 2 B 3 C 1):",
            foreground="#444444",
        ).pack(anchor="w")
        mission_var = tk.StringVar(value="A 1 B 2 C 3")
        ttk.Entry(mission_frame, textvariable=mission_var).pack(
            fill=tk.X, pady=(2, 6)
        )
        ttk.Button(
            mission_frame, text="Set Mission",
            command=lambda n=name, v=mission_var:
                self._send(f"DRONE_MISSION {n} {v.get()}"),
        ).pack(fill=tk.X)
