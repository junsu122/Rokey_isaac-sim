...
_CRITICAL_AVOID_DIST = 0.55  # 이보다 가까워도 즉시 우회 목표를 만들고 천천히 이동
_YIELD_WAIT_STEPS = 0        # 정지 대기 없이 우회 주행
_HUB_AVOID_DIST  = 2.2   # IW Hub 주변에서 Spot 이 우회 시작하는 거리 [m]
...
def _clamp_clear_of_pods(xy: np.ndarray) -> np.ndarray:
    ...
    x = max(-_WALL_SAFE_X, min(_WALL_SAFE_X, float(xy[0])))
    y = max(-_WALL_SAFE_Y, min(_WALL_SAFE_Y, float(xy[1])))
    ...
    for x0, x1, y0, y1 in _CONVEYOR_ZONES:
        if x0 < x < x1 and y0 < y < y1:
            ...
            return _clamp_clear_of_pods(
                min(candidates, key=lambda p: float(np.linalg.norm(p - xy[:2]))))
...
def _pod_safe_detour(pos_xy: np.ndarray, target_xy: np.ndarray) -> np.ndarray:
    ...
    if abs(pos_xy[0]) < _POD_ZONE_HALF_X + 0.3:
        return _clamp_clear_of_pods(np.array([safe_x, float(pos_xy[1])], dtype=np.float64))
    return _clamp_clear_of_pods(np.array([safe_x, target_xy[1]], dtype=np.float64))
...
class SpotAgent(BaseRobotAgent):
    ...
    def _init_internal_state(self, spawn):
        ...
        self._active_hub_detour = None   # IW Hub 회피용 우회 목표점
        self._active_yield_detour = None # 상위 Spot 회피용 우회 목표점
        self._yield_wait_cnt    = 0
    ...
    def _init_finger_links(self):
        ...
        self._grip_center_xforms = [
            UsdGeom.Xformable(self.world.stage.GetPrimAtPath(f"{self._gripper_path}/{ln}"))
            for ln in ("right_inner_finger", "left_inner_finger")
            if self.world.stage.GetPrimAtPath(f"{self._gripper_path}/{ln}").IsValid()
        ]
    ...
    def _get_grip_center(self) -> np.ndarray:
        pts = []
        for xf in self._grip_center_xforms:
            try:
        ...
        if world.IsValid():
            for prim in world.GetChildren():
                if str(prim.GetPath()).startswith("/World/AutoBox_"):
                    yield prim
        for root_path in ("/World/DynamicBoxes", "/World/MinimapClickBoxes"):
    ...
    def _find_nearest_autobox(self, spot_xy: np.ndarray,
        ...
        for prim in self._iter_box_prims():
    ...
    def _attach_nearest_autobox(self) -> bool:
        """
        ...
        """
        try:
            grip_xy = self._get_grip_center()[:2]
            path, _ = self._find_nearest_autobox(
                grip_xy, max_dist=1.6, skip_goal_zones=True)
            if path is None:
                pos, _ = self._get_pos_yaw()
                path, _ = self._find_nearest_autobox(
                    pos[:2], max_dist=2.0, skip_goal_zones=True)
            if path is None:
                print(f"[{self.name}] 근처 AutoBox 없음")
                return False
            stage = omni.usd.get_context().get_stage()
    ...
    def _detach_autobox(self):
        """Place the carried AutoBox on the ID0 ArUco marker zone."""
        if self._grip_box_path is None:
            return
        released_path = self._grip_box_path
        stage = omni.usd.get_context().get_stage()
        prim  = stage.GetPrimAtPath(released_path)
        goal = np.array(self._aruco_goals.get(0, (-2.8, 14.4)), dtype=np.float64)
        if prim.IsValid() and prim.HasAPI(UsdPhysics.RigidBodyAPI):
            place_z = 0.15
            xf = UsdGeom.Xformable(prim)
            for op in xf.GetOrderedXformOps():
                if op.GetOpType() == UsdGeom.XformOp.TypeTranslate:
                    op.Set(Gf.Vec3d(float(goal[0]), float(goal[1]), float(place_z)))
                    break
            else:
                xf.ClearXformOpOrder()
                xf.AddTranslateOp().Set(Gf.Vec3d(float(goal[0]), float(goal[1]), float(place_z)))
        ...
        print(f"[{self.name}] AutoBox ID0 zone 배치 완료: {released_path}  goal={goal.round(2)}")
        self._grip_box_path = None
    ...
    def _select_goal_for_box(self, aruco_id: int, box_path: str = None) -> np.ndarray:
        """Always send Spot-delivered boxes to the ID0 ArUco marker zone."""
        return np.array(self._aruco_goals.get(0, (-2.8, 14.4)), dtype=np.float64)
    ...
    def _nav_toward(self, tgt_xy: np.ndarray, speed: float = None) -> np.ndarray:
        ...
        try:
            ...
            if not np.allclose(escape, pos[:2], atol=0.05):
                escape = _clamp_clear_of_pods(escape)
                tgt_xy = escape
            else:
            ...
            turn_scale = max(0.18, math.cos(min(abs(ey), math.pi / 2.0)))
            vx       = speed * min(1.0, dist / _APPROACH_DIST) * turn_scale
    ...
    def _waypoint_cmd(self) -> np.ndarray:
        try:
            ...
            escape = _clamp_clear_of_pods(pos[:2])
            if not np.allclose(escape, pos[:2], atol=0.05):
                return self._nav_toward(escape)
    ...
    def _active_goal_xy(self) -> np.ndarray | None:
        if self._state == "NAVIGATE_TO_CUBE" and self._cube_nav is not None:
            return self._cube_nav
        if self._state == "NAVIGATE_TO_GOAL" and self._goal_xy is not None:
            return self._goal_xy
        if self._waypoints:
            return self._waypoints[self._wp_idx]
        return None

    def _compute_detour_around_higher(self) -> tuple[np.ndarray | None, bool]:
        """Return detour around a higher-priority Spot, plus critical-stop flag."""
        try:
            my_idx = SpotAgent._registry.index(self)
            my_pos, _ = self._get_pos_yaw()
            active_goal = self._active_goal_xy()
            ...
            for other in SpotAgent._registry:
                ...
                if dist >= _AVOID_DIST:
                    continue
                tgt_dir = np.array(active_goal, dtype=np.float64) - my_pos[:2]
                ...
                return (
                    _clamp_clear_of_pods(np.array(detour, dtype=np.float64)),
                    dist < _CRITICAL_AVOID_DIST,
                )
        except Exception:
            pass
        return None, False
        ...
        try:
            ...
            for other in SpotAgent._registry:
                ...
                if dist < _AVOID_DIST:
                    active_goal = None
                    if self._state == "NAVIGATE_TO_CUBE" and self._cube_nav is not None:
                        active_goal = self._cube_nav
                    elif self._state == "NAVIGATE_TO_GOAL" and self._goal_xy is not None:
                        active_goal = self._goal_xy
                    elif self._waypoints:
                        active_goal = self._waypoints[self._wp_idx]
                    if active_goal is None:
                        return None
                    tgt_dir = np.array(active_goal, dtype=np.float64) - my_pos[:2]
                    nd = np.linalg.norm(tgt_dir)
    ...
    def _get_prim_xy(self, prim_path: str) -> np.ndarray | None:
        try:
            stage = omni.usd.get_context().get_stage()
            prim = stage.GetPrimAtPath(prim_path)
            if not prim.IsValid():
                return None
            tr = UsdGeom.XformCache().GetLocalToWorldTransform(prim).ExtractTranslation()
            return np.array([float(tr[0]), float(tr[1])], dtype=np.float64)
        except Exception:
            return None

    def _compute_detour_around_hub(self) -> np.ndarray | None:
        """Return a local detour if this Spot is close to an IW Hub."""
        try:
            my_pos, _ = self._get_pos_yaw()
            active_goal = None
            if self._state == "NAVIGATE_TO_CUBE" and self._cube_nav is not None:
            ...
            for cfg in C.ROBOT_REGISTRY:
                if cfg.get("type") != "iw_hub":
                    continue
                hub_xy = self._get_prim_xy(f"/World/{cfg['name']}")
                if hub_xy is None:
                    continue
                dist = float(np.linalg.norm(my_pos[:2] - hub_xy))
                if dist >= _HUB_AVOID_DIST:
                    continue
                ...
                perp1 = np.array([-tgt_dir[1], tgt_dir[0]])
                perp2 = -perp1
                diff = my_pos[:2] - hub_xy
                perp = perp1 if np.dot(perp1, diff) >= np.dot(perp2, diff) else perp2
                detour = hub_xy + perp * _DETOUR_SIDE + tgt_dir * _DETOUR_FWD
                return _clamp_clear_of_pods(np.array(detour, dtype=np.float64))
        except Exception:
            pass
        return None
    ...
    def _run_fsm(self) -> np.ndarray:
        ...
        if self._state in ("WALKING", "NAVIGATE_TO_CUBE", "NAVIGATE_TO_GOAL"):
            # 하위 우선순위: 너무 가까울 때만 짧게 정지하고, 그 외에는 우회해서 계속 이동
            if self._should_yield():
                if self._gripped:
                    self._sync_autobox_to_gripper()
                self._yield_wait_cnt += 1
                if self._active_yield_detour is not None:
                    pos, _ = self._get_pos_yaw()
                    if np.linalg.norm(pos[:2] - self._active_yield_detour) <= _LOOK_AHEAD:
                        self._active_yield_detour = None
                    else:
                        return self._nav_toward(self._active_yield_detour, speed=0.38)

                detour, critical = self._compute_detour_around_higher()
                if detour is not None and (not critical or self._yield_wait_cnt >= _YIELD_WAIT_STEPS):
                    self._active_yield_detour = detour
                    return self._nav_toward(detour, speed=0.38)
                if critical:
                    active_goal = self._active_goal_xy()
                    if active_goal is not None:
                        return self._nav_toward(active_goal, speed=0.18)
                active_goal = self._active_goal_xy()
                if active_goal is not None:
                    return self._nav_toward(active_goal, speed=0.32)
            ...
            pos, _ = self._get_pos_yaw()
            if self._active_hub_detour is not None:
                if np.linalg.norm(pos[:2] - self._active_hub_detour) <= _LOOK_AHEAD:
                    self._active_hub_detour = None
                else:
                    if self._gripped:
                        self._sync_autobox_to_gripper()
                    return self._nav_toward(self._active_hub_detour)
            hub_detour = self._compute_detour_around_hub()
            if hub_detour is not None:
                self._active_hub_detour = hub_detour
                if self._gripped:
                    self._sync_autobox_to_gripper()
                return self._nav_toward(hub_detour)
        ...
        if self._state == "WALKING":
            ...
            if self._ganim_state == "idle":
                if not self._attach_nearest_autobox():     # AutoBox 흡착
                    self._gripped         = False
                    self._grip_box_path   = None
                    self._cube_nav        = None
                    self._goal_xy         = None
                    self._state           = "WALKING"
                    self._state_step      = 0
                    self._pickup_cooldown = 120
                    self._set_nearest_patrol_waypoint()
                    print(f"[{self.name}] 픽업 실패 → WALKING")
                    return cmd
                self._gripped = True
                self._goal_xy = np.array(self._aruco_goals.get(0, (-2.8, 14.4)),
                                         dtype=np.float64)
                self._sync_autobox_to_gripper()
                self._state      = "RAISE"
                self._state_step = 0
            ...
            # Always go to ID0 marker; never release early in the section.
            self._goal_xy = np.array(self._aruco_goals.get(0, (-2.8, 14.4)),
                                     dtype=np.float64)
            goal = self._goal_xy
            ...
            try:
                pos, _ = self._get_pos_yaw()
                if np.linalg.norm(pos[:2] - goal) < 0.45:
                    self._state      = "RELEASE"
                    self._state_step = 0
                    self._trigger_open()
