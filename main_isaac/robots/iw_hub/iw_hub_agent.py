...
_SPOT_WAIT_DIST = 0.50
_SPOT_STOP_HUBS = {"iw_hub_01"}
...
class IwHubAgent(SectionAFSM, SectionBFSM, SectionCFSM, PickupFSM, StandardFSM, BaseRobotAgent):
    ...
    def setup(self) -> None:
        ...
        self._path_wps        = []
        self._path_wp_idx     = 0
        self._spot_stop_dist  = float(self.cfg.get("spot_stop_distance", _SPOT_WAIT_DIST))
        self._stop_for_spot   = bool(self.cfg.get("stop_for_spot", self.name in _SPOT_STOP_HUBS))
        self._spot_safety_active = False
    ...
    def _iter_spot_xy(self):
        """Yield current Spot root positions in world XY."""
        seen = set()
        try:
            from robots.spot.spot_agent import SpotAgent
            for agent in getattr(SpotAgent, "_registry", []):
    ...
    def _spot_too_close(self) -> bool:
        """Pause only when a Spot is within the configured stop distance.

        Uses USD ground-truth position for the hub (not odom) so drift never
        causes a false positive stop when the Spot is farther than the limit.
        """
        if not self._stop_for_spot:
            return False
        try:
            world_pose = self._get_world_xy_hdg()
            if world_pose is None:
                return False
            hx, hy, _ = world_pose
            for name, pos in self._iter_spot_xy():
                sx, sy = pos
                dist = math.hypot(sx - hx, sy - hy)
                if dist <= self._spot_stop_dist:
                    self._spot_wait_dbg = getattr(self, "_spot_wait_dbg", 0) + 1
                    if self._spot_wait_dbg % 100 == 1:
                        print(f"[{self.name}] Spot within {self._spot_stop_dist:.2f}m, stopping  "
    ...
    def _publish_cmd_vel(self, lv: float, av: float) -> None:
        ...
        if self._ros2_cmd_pub is None:
            ...
            return
        requested_motion = abs(lv) > 1e-4 or abs(av) > 1e-4
        if requested_motion and self._spot_too_close():
            self._spot_safety_active = True
            lv, av = 0.0, 0.0
        elif requested_motion and self._spot_safety_active:
            self._spot_safety_active = False
            print(f"[{self.name}] Spot clear beyond {self._spot_stop_dist:.2f}m, resuming")
        try:
            from geometry_msgs.msg import Twist
            msg = Twist()
