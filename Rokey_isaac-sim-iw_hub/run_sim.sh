...
# 내장 ROS2 shared lib 경로 추가 (check_status 통과 및 rmw 로드를 위해)
export LD_LIBRARY_PATH="${BRIDGE_EXT}/humble/lib:${LD_LIBRARY_PATH}"

export ROS_DISTRO=humble
export RMW_IMPLEMENTATION=rmw_fastrtps_cpp

echo "[run_sim] PYTHONPATH 앞에 내장 rclpy 추가 완료"
echo "[run_sim] LD_LIBRARY_PATH에 bridge lib 추가 완료"
